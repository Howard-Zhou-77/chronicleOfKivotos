...
game.import('character', function (lib, game, ui, get, ai, _status) {
	...
	var ba_characters = {
		...
		characterSort: {
			/*
			分包信息
			tip:包名和子包名都需要在translate中注明中文翻译！
			...
			e.p
			example:{
				example_name:['character1','character2',...],
			},
			*/
		},
		characterTitle: {
		...
		character: {
			...
			武将名:["性别","势力","开局体力/体力上限/护甲值",["技能1",...],["特殊标签，如主公就写'zhu'"]],
			...
			*/
			...
			ba_KikyouKiryuu: ['female', 'jin', 4, ['ba_mouju', 'ba_eyan', 'ba_dingji'], ['hiddenSkill']],
		...
		skill: {
			...
			*/
					...
					call: {
						...
						content: () => {
							...
							if (event.opt.length == 0) event.finish();
								...
								if (event.opt.includes('闪') && player.countCards('h', { name: 'shan' }) == 0) return '闪';
								if (event.opt.includes('杀') && player.countCards('h', { name: 'sha' }) == 0) return '杀';
								if (event.opt.includes('装备') && player.countCards('h', card => { return get.type(card) == 'equip' }) == 0) return '装备';
			...
			"ba_zhimeng": {
				...
				content: (event, step, source, player, targets, card, cards, skill, forced, num, trigger, result) => {
					...
					for (var i of result.moved[1]) {
						if (!ts.includes(i)) cards2.push(i);
					}
					if (cards1.length) {
			...
			"ba_yangmou": {
				...
				content: (event, step, source, player, targets, card, cards, skill, forced, num, trigger, result) => {
					...
					if (targets1.length > 1) player.chooseTarget(true, '选择一名角色进行拼点' + (player.storage.ba_yangmou + 1).toString() + '/3', '若你赢，则获得其一张牌' + ((player.storage.ba_yangmou < 2) ? '' : '且使命成功') + '；若你未赢且点数小于其手牌数与体力值之和同10的最小值，使命失败。', (card, player, target) => {
						return targets1.includes(target);
					}).set('ai', target => {
						var player = get.player();
			...
			"ba_xuansuan": {
				...
				filter: (event, player) => {
					return event.source && event.source != player && event.player.isAlive() && (event.player == player || player.inRange(event.player)) && (get.itemtype(event.cards) == 'cards' && get.position(event.cards[0], true) == 'o' || event.card && get.position(event.card, true) == 'o')
				},
			...
			"ba_mieju": {
				...
				content: function () {
					'step 0'
					player.chooseTarget(get.prompt('ba_mieju'), '对一名本回合失去过牌的其他角色造成1点振动伤害', function (card, player, target) {
						...
						var player = _status.event.player;
						// return get.damageEffect(target, player, player);
						return get.effect(target, { name: "ba_sonicdamage" }, player, player)
					});
					'step 1'
					if (result.bool) {
						var target = result.targets[0];
						player.logSkill('ba_mieju', target);
						target.damage('ba_sonic');
					}
				},
				group: "ba_mieju_1",
				subSkill: {
				...
				subSkill: {
					draw: {
						...
						filter: (event, player) => {
							...
							return event.player.getHistory('lose', evt => {
								...
								for (var i in evt.gaintag_map) {
									if (evt.gaintag_map[i].includes('ba_xinzhu')) return true;
								}
								return false;
							}).length > 0
						},
						...
						content: () => {
							...
							for (var evt of his) {
								for (var i in evt.gaintag_map) {
									if (evt.gaintag_map[i].includes('ba_xinzhu')) num++;
								}
							}
			...
			"ba_daohui": {
				...
				subSkill: {
					g: {
						...
						filter: (event, player) => {
							return player.isPhaseUsing() && get.suit(event.card) && ['spade', 'club'].includes(get.suit(event.card)) && game.hasPlayer(target => target != player && target.hasSkill('ba_daohui'));
						},
						direct: true,
						content: (event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) => {
							'step 0'
			...
			"ba_yizhen": {
				...
				filter: (event, player) => {
					return event.card.name == 'sha' && game.hasPlayer(target => !event.targets.includes(target) && player.canUse(event.card, target));
				},
				direct: true,
				content: (event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) => {
					'step 0'
					player.chooseTarget(get.prompt(event.name), 1, '为' + get.translation(trigger.card) + '增加一个目标', (card, player, target) => !_status.event.sourcex.includes(target) && player.canUse(_status.event.card, target)).set('sourcex', trigger.targets).set('card', trigger.card).set('ai', target => {
						var player = _status.event.player;
			...
			"ba_qiangfeng": {
				...
				subSkill: {
					count: {
						...
						filter: function (event, player, name) {
							...
							if (player.storage.ba_qiangfeng && player.storage.ba_qiangfeng.includes(suit)) return false;
							return true;
						},
						content: function () {
				...
				filter: (event, player) => {
					...
					var num = 0;
					for (var card of cards) {
						if (get.type(card) != 'equip' && player.storage.ba_qiangfeng.includes(get.suit(card))) num++;
					}
				...
				content: (event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) => {
					'step 0'
					player.chooseCardTarget({
						filterCard: (card, player) => { return get.type(card) != 'equip' && player.storage.ba_qiangfeng.includes(get.suit(card)) },
			...
			"ba_pushu": {
				...
				filter: (event, player) => get.number(event.card) && [1, 11, 12, 13].includes(get.number(event.card)) && get.color(event.card) && (get.color(event.card) == 'red' || (get.color(event.card) == 'black' && player.canMoveCard())),
				frequent: true,
				content: () => {
			...
			"ba_tianjian": {
				...
				forced: true,
				filter: function (event, player) {
					return (event.name != 'phase' || game.phaseNumber == 0) && !lib.inpile.includes('ba_HikarinoKen');
				},
				content: () => {
			...
			"ba_jixue": {
				...
				init: (player, skill) => { player.storage[skill] = []; },
				hiddenCard: (player, name) => player.countCards('h') > 0 && player.getStorage('ba_jixue').includes(name),
				onremove: true,
				chooseButton: {
					dialog: (event, player) => {
						var vcards = [];
				...
				subSkill: {
					record: {
						...
						filter: (event, player) => event.player != player && player.inRange(event.player) && !player.getStorage('ba_jixue').includes(get.name(event.card)) && player.getStorage('ba_jixue').length <= 2 && player.countCards('hes') > 0 && ['basic', 'trick'].includes(get.type(event.card)),
						content: () => {
							'step 0'
			...
			ba_jijie: {
				...
				content: () => {
					...
					if (result.bool) {
						...
						if (['trick', 'basic', 'delay'].includes(get.type(result.cards[0]))) {
			...
			'ba_pojia': {
				...
				filter: function (event) {
					return event.card.name == 'sha' && event.player.countCards('e', { name: "tengjia" }) == 0 && !event.player.hasSkill('ba_shedie_tengjia');
				},
				forced: true,
				logTarget: "target",
				content: function () {
				...
				ai: {
					"unequip_ai": true,
					skillTagFilter: function (player, tag, arg) {
						if (arg && arg.name == 'sha' && arg.target && arg.target.countCards('e', { name: "tengjia" }) == 0 && !arg.target.hasSkill('ba_shedie_tengjia')) return true;
						return false;
					},
				},
				locked: true,
				...
				subSkill: {
					1: {
						...
						filter: (event, player) => {
							return (!player.isDamaged() || player.countCards('e') > 0 || player.countCards('h') > 2)
								&& ((event.player.countCards('e', { name: "tengjia" }) == 0 && !event.player.hasSkill('ba_shedie_tengjia')) || (event.nature && event.nature == 'fire'))
						},
						forced: true,
						sub: true,
						locked: true,
						content: () => {
							if (trigger.player.countCards('e', { name: "tengjia" }) == 0 && !trigger.player.hasSkill('ba_shedie_tengjia')) trigger.player.addTempSkill('ba_pojia_3')
					...
					3: {
						ai: {
							"unequip2": true,
							skillTagFilter: (player, tag, arg) => {
								return !(player.getEquip(2) && get.name(player.getEquip(2)).includes('tengjia')) && !player.hasSkill('ba_shedie_tengjia')
							}
						},
						sub: true,
						mark: true,
				...
				mod: {
					cardname: (card, player) => {
						if (get.position(card) != 'e' && get.position(card) != 'j') {
							...
							if (pl.length > 0) {
								if (pl.includes(player)) {
									if (player.storage.ba_duyan_1.includes(get.suit(card))) return 'du';
								}
								for (var _pl of pl) {
									if (_pl.storage.ba_duyan == player) {
										if (_pl.storage.ba_duyan_1.includes(get.suit(card))) return 'du';
									}
								}
							}
						}
			...
			'ba_ekou': {
				...
				content: (event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) => {
					...
					for (var _event of damaged) {
						if (damaged_players.includes(_event.player)) {
			...
			ba_zhajing: {
				...
				content: (event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) => {
					...
					if (result.control != 'cancel2') {
						...
						if (!['选项三', '选项四'].includes(result.control)) event.finish();
				...
				subSkill: {
					...
					baiban: {
						...
						skillBlocker: function (skill, player) {
							if (!(player.getEquip(2) && player.getEquip(2).name == 'tengjia') && lib.card.tengjia.skills.includes(skill)) return false;
				...
				content: (event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) => {
					...
					player.chooseTarget([1, 3], get.prompt(event.name), (card, player, target) => {
						...
						if (get.color(trigger.card) == 'red') {
							...
							if (['nanman', 'wanjian', 'cxy_shuiyanqijun'].includes(get.name(trigger.card)) || (get.name(trigger.card) == 'sha' && !get.nature(trigger.card))) {
								return -get.damageEffect(target, player, player)
					...
					if (result.bool) {
						...
						for (var target of result.targets) {
							if (get.color(trigger.card) == 'red') {
								...
								if (equip2) {
									target.addSkill('ba_shedie_tengjia')
									...
									next.setContent(function () {
										player.removeSkill('ba_shedie_tengjia')
			...
			ba_jixing: {
				...
				content: (event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) => {
					...
					if (result.bool) {
						event.target = result.targets[0];
						player.loseHp()
						player.gainPlayerCard(event.target, 1)
						player.draw()
						event.target.addTempSkill('ba_jixing_1')
						event.target.storage.ba_jixing_1 = player;
					}
				},
				mod: {
			...
			ba_yuyun: {
				trigger: { player: "phaseZhunbeiBegin" },
				filter: (event, player) => {
					if (!lib.inpile.includes('ba_yuyunhao')) return true;
				...
				content: (event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) => {
					var card;
					if (!lib.inpile.includes('ba_yuyunhao')) {
					...
					player.addTempSkill('ba_yuyun_1')
			...
			'ba_mouju': {
				...
				filter(event, player) {
					return player.countCards('h', card => {
						return get.type2(card) == 'trick' && get.name(card) != 'wuxie'
					}) > 0 && game.hasPlayer(target => {
				...
				content(event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) {
					...
					player.chooseCardTarget({
						...
						prompt: get.prompt(event.name),
						prompt2: '弃置一张不为【无懈可击】的锦囊牌并令其选择：弃置一张同名锦囊牌/【无懈可击】，或受到你的1点振动伤害且直到下一回合开始其受的所有伤害+1，同时无属性伤害都视为振动伤害。',
						ai1(card) {
							return 7 - get.value(card)
						},
						ai2(target) {
					...
					'step 1'
					if (result.bool) {
						event.cardname = get.name(result.cards[0])
						player.discard(result.cards)
						player.logSkill(event.name, result.targets)
						event.target = result.targets[0]
						event.target.chooseToDiscard(card => get.name(card) == _status.event.cardname || get.name(card) == 'wuxie', 1, 'h',
							'弃置一张【' + get.translation(event.cardname) + '】或【' + get.translation('wuxie') + '】，否则受到1点振动伤害且直到' +
							get.translation(player) + '下回合开始受到的伤害+1，同时无属性伤害均视为振动伤害', card => {
								let player = _status.event._player;
								let target = _status.event._target;
								if (get.effect(target, { name: 'ba_sonicdamage' }, player, target) > 0) return -1;
								else return 7 - get.value(card)
							}).set('cardname', event.cardname).set('_target', event.target).set('_player', player)
					} else event.finish();
					'step 2'
					if (!result.bool) {
						event.target.damage('ba_sonic')
						player.storage.ba_mouju_1 = event.target;
						player.syncStorage('ba_mouju_1')
					}
				},
				group: ['ba_mouju_1', 'ba_mouju_2'],
				subSkill: {
					1: {
						intro: {
							content(storage, player, skill) {
								if (player.storage.ba_mouju_1) {
									return '当前目标角色为' + get.translation(player.storage.ba_mouju_1)
								} else return '无目标角色'
							}
						},
						mark: true,
						...
						content() {
							if (player.storage[event.name]) {
								player.storage[event.name] = undefined
								player.syncStorage(event.name)
							}
						},
						popup: false,
					...
					2: {
						...
						firstDo: true,
						filter(event, player) {
							return player.storage.ba_mouju_1 && event.player == player.storage.ba_mouju_1
						},
						content(event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) {
							trigger.num++;
							if (!trigger.hasNature()) {
								game.setNature(trigger, 'ba_sonic')
								game.log(trigger.source, '对', trigger.player, '的伤害被视作振动属性。')
							}
						},
						forced: true,
						locked: true,
			...
			'ba_eyan': {
				...
				content(event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) {
					'step 0'
					trigger.player.showHandcards()
					let maxnum = Math.max(...trigger.player.getCards('h').map(card => get.number(card)))
					event.tricks = trigger.player.getCards('h', card => get.type2(card) == 'trick')
					if (event.tricks.length > 0 && player.canCompare(trigger.player)) {
						player.chooseBool('是否与' + get.translation(trigger.player) + '拼点？若你赢，你获得其一张锦囊牌且其本回合不得使用或打出手牌；若你输，你结束出牌阶段。')
							.set('ai', () => {
								let max1 = _status.event.max1;
								let max2 = _status.event.max2;
								return max1 > max2;
							})
							.set('max2', maxnum).set('max1', Math.max(...player.getCards('h').map(card => ((7 - get.value(card) > 0) ? get.number(card) : get.number(card) - 1))))
					} else event.finish()
					'step 1'
					if (result.bool) {
					...
					'step 2'
					if (result.bool) {
						trigger.player.addTempSkill('ba_eyan_1');
						if (trigger.player.countCards('h', card => get.type2(card) == 'trick'))
							player.gainPlayerCard(trigger.player, 1, 'visible', 'h', button => get.value(button.link), button => get.type2(button.link) == 'trick')
					} else {
			...
			'ba_dingji': {
				...
				content(event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) {
					'step 0'
					_status.currentPhase.chooseCard('定计：请选择一张牌交给' + get.translation(player) + '，否则其从牌堆内获得一张锦囊牌。（*此时若牌堆中没有锦囊，你受到其1点伤害）',
						'h', 1).set('ai', card => {
							if (get.attitude(_status.currentPhase, _status.event._player) > 0) return -1;
							return 4 - get.value(card)
						}).set('_player', player);
					'step 1'
					if (result.bool) {
						_status.currentPhase.give(result.cards, player)
					} else {
						let card = get.cardPile(function (card) {
							return get.type2(card) == 'trick';
						});
						if (card) player.gain(card, 'gain2');
						else _status.currentPhase.damage('ba_sonic')
					}
				}
			},
			'ba_fengping': {
				...
				filter: (event, player) => {
					return event.player != player && event.player.isPhaseUsing() && get.number(event.card) && player.countCards('h') && get.type(event.card) != 'equip';
				},
				direct: true,
				popup: false,
				usable: 1,
				content(event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) {
					'step 0'
					player.chooseToDiscard('h', get.prompt2(event.name)).set('ai', card => {
						let target = _status.event.target;
						let player = _status.event.player;
						let number = _status.event.number;
						if (get.number(card) < number) return get.effect(target, { name: "losehp" }, player, player) - 0.1 * get.value(card);
						else if (get.number(card) == number) return (1 + _status.event.isSha ? 1 : 0) * Math.sign(get.attitude(player, target)) - 0.1 * get.value(card)
						else return Math.sign(get.attitude(player, target)) / 2 + 0.1 * (_status.event.value - get.value(card))
					}).set('target', trigger.player).set('number', get.number(trigger.card)).set('isSha', get.name(trigger.card) == 'sha').set('value', get.value(trigger.card, player));
					'step 1'
					if (result.bool) {
						player.logSkill(event.name, trigger.player);
						let sgn = Math.sign(get.number(result.cards[0]) - get.number(trigger.card));
						switch (sgn) {
							case 1:
								trigger.player.loseHp();
								break;
							case 0:
								game.log(get.translation(trigger.card), '不计入', trigger.player, '的使用次数。')
								trigger.addCount = false;
								if (trigger.getStat().card[get.name(trigger.card)]) trigger.getStat().card[get.name(trigger.card)]--;
								trigger.player.draw(2);
								break;
							default:
								trigger.player.draw();
								player.gain(trigger.cards, 'draw');
						}
					} else {
						if (player.storage.counttrigger && player.storage.counttrigger[event.name] > 0) player.storage.counttrigger[event.name]--;
					}
				}
			},
			'ba_zhanhui': {
				...
				usable: 1,
				filter: function (event, player) {
					return game.hasPlayer((current) => current.countCards('h') > 0 && player != current) && player.countCards('h') > 0;
				},
				filterTarget: function (card, player, target) {
				...
				content: function (event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) {
					...
					game.broadcastAll(function (targets, cards, id, player) {
						var dialog = ui.create.dialog(get.translation(player) + '发动了【展会】', cards);
						dialog.videoId = id;
					...
					let list = [], map = [];
					cards = event.cards.slice(0).sort((a, b) => get.number(a) - get.number(b))
					list = cards.slice(0);
					if (get.number(list[0]) != get.number(list[1])) list.shift(0);
					map = list.map(card => event.cards.indexOf(card))
					let targetsx = [];
					for (let index of map) {
						targetsx.push(targets[index]);
						targets[index].addTempSkill('ba_zhanhui_1', { player: ["phaseUseEnd"] });
					}
			...
			'ba_chengpai': {
				...
				content(event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) {
					...
					player.showCards(event.cards);
					let suit = get.suit(event.cards)
					game.log(player, '展示了', suit, '花色的牌');
					// player.storage[event.name] = suit;
				...
				group: ['ba_chengpai_draw', 'ba_chengpai_clear'],
				onremove: (player, skill) => {
				...
				subSkill: {
					draw: {
						...
						trigger: { player: "useCard" },
						filter(event, player) {
							if (player.storage.ba_chengpai) {
								if (get.suit(event.card) && player.storage.ba_chengpai == get.suit(event.card)) return true;
								return get.number(event.card) && get.number(event.card) != 0 && (get.number(event.card) % 7 == 0 || get.number(event.card) % 5 == 0)
							} else return false;
						},
						content: () => {
							player.draw();
							if (player == _status.currentPhase) player.addMark(event.name, 1, false);
						},
						mod: {
							cardUsable: (card, player, num) => {
								if (get.name(card) == 'sha') return num + player.countMark('ba_chengpai_draw');
							}
						},
						onremove: true,
						intro: { content: "出杀次数+#" },
					...
					clear: {
						...
						sub: true,
						content: () => {
							if (player.storage.ba_chengpai && event.triggername == 'phaseBegin') {
								player.unmarkSkill('ba_chengpai')
								player.storage.ba_chengpai = undefined;
							} else if (player.countMark('ba_chengpai_draw') && event.triggername == 'phaseEnd') {
								player.removeMark('ba_chengpai_draw', player.countMark('ba_chengpai_draw'), false)
								player.unmarkSkill('ba_chengpai_draw');
							}
						}
					}
				}
			...
			'ba_qianji': {
				trigger: { target: "useCardToTargeted" },
				filter(event, player) {
					return player != event.player && player.inRange(event.player) && event.cards
						&& get.color(event.cards) && get.color(event.cards) != 'none'
						&& player.countCards('h', { color: get.color(event.cards) });
				},
				direct: true,
				popup: false,
				content(event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) {
					'step 0'
					player.chooseCard(get.prompt2(event.name), { color: get.color(trigger.cards) }, 'he', [1, Infinity]).set('ai', card => {
						let _card = _status.event.card;
						let target = _status.event.target;
						let player = _status.event.player;
						return -Math.sign(get.effect(player, _card, target, player))
					}).set('card', trigger.card).set('target', trigger.player)
					'step 1'
					if (result.bool) {
						event.cards = result.cards;
						player.chooseToDiscard('请弃置一张展示的牌。', 1, 'he', card => {
					...
					if (result.bool) {
						player.logSkill(event.name, trigger.player);
						player.showCards(event.cards);
						if (trigger.player.countCards('h') > event.cards.length) {
							event._result = { bool: false };
						} else {
							trigger.player.chooseToDiscard('h', event.cards.length,
								'请弃置' + get.cnNumber(event.cards.length) + '张牌，否则' + get.translation(trigger.card) + '对' + get.translation(player) + '无效。')
								.set('ai', card => {
									let player = _status.event._player;
									let target = _status.event.target;
									let _card = _status.event.card;
									if (get.effect(target, _card, player, player) * get.attitude(player, target) > 0) {
										return 6 - get.value(card);
									} else return -1;
								}).set('_player', trigger.player).set('target', player).set('card', trigger.card)
						}
					} else event.finish();
					'step 3'
					if (result.bool) {
						game.log(trigger.player, '的', trigger.card, '对', player, '继续结算。');
					} else {
						trigger.getParent().excluded.add(player);
						trigger.player.addSkill('ba_qianji_2');
						trigger.player.addToExpansion(trigger.cards, player, 'gain').gaintag.add('ba_qianji_2');
						player.storage.ba_qianji.push(trigger.player);
					}
				},
				init(player, skill) {
					player.storage[skill] = [];
				...
				subSkill: {
					1: {
						trigger: { player: "phaseBegin" },
						filter: (event, player) => {
							return player.storage.ba_qianji && player.storage.ba_qianji.length > 0
								&& game.hasPlayer2(current => player.storage.ba_qianji.includes(current))
						},
						forced: true,
						// charlotte: true,
						// locked: true,
						...
						content(event, step, source, player, target, targets, card, cards, skill, forced, num, trigger, result) {
							'step 0'
							let _targets = game.filterPlayer(current => player.storage.ba_qianji.includes(current) && current.hasSkill('ba_qianji_2') && current.getExpansions('ba_qianji_2').length > 0);
							event.targets = [];
							if (_targets.length) {
								for (let _target of _targets) {
									if (_target.getExpansions('ba_qianji_2').length > 0) {
										player.gain(_target, _target.getExpansions('ba_qianji_2'), 'give');
										event.targets.add(_target)
									}
									_target.removeSkill('ba_qianji_2');
								}
							}
			...
			'ba_poyan': {
				...
				filterTarget: (card, player, target) => player != target && player.inRange(target),
				filterCard: (card, player) => {
					return !ui.selected.cards.some(cardx => get.number(cardx, player) % 2 == get.number(card, player) % 2)
				},
				complexCard: true,
				check: card => {
					return 6 - get.value(card)
				...
				content() {
					player.discardPlayerCard(target, 1)
					target.damage('ba_sonic');
				},
				position: 'he',
				ai: {
					result: {
						target: (player, target) => {
							return get.effect(target, { name: "ba_sonicdamage" }, player, target)
						},
						player: (player, target) => {
							return get.effect(target, { name: "ba_sonicdamage" }, player, player)
						},
			...
			'ba_tuolong': {
				trigger: { source: "damageEnd" },
				filter(event, player) {
					return event.hasNature('linked') && event.num > 0
				},
				frequent: true,
				content() {
					player.addMark(event.name, trigger.num);
				...
				group: ['ba_tuolong_use'],
				subSkill: {
					use: {
						enable: ['chooseToUse', 'chooseToRespond'],
						prompt: "弃置1枚“泉”，将一张点数为奇数/偶数的牌当作【闪】/【无懈可击】使用或打出。",
						viewAs(cards, player) {
							let isOdd = get.number(cards[0], player) % 2
							let name;
							if (isOdd == 0) {
								name = 'wuxie'
							} else name = 'shan';
						...
						filter(event, player) {
							if (player.countMark('ba_tuolong') == 0) return false;
							let filter = event.filterCard;
							if (filter({ name: 'shan' }, player, event)
								&& player.countCards('hes', card => get.number(card) % 2 == 1)
								&& !player.hasSkill('ba_tuolong_shan'))
								return true;
							if (filter({ name: 'wuxie' }, player, event)
								&& player.countCards('hes', card => get.number(card) % 2 == 0)
								&& !player.hasSkill('ba_tuolong_wuxie'))
								return true;
							return false;
						},
						position: "hes",
						...
						filterCard(card, player, event) {
							...
							let isOdd = get.number(card) % 2 == 1;
							if (isOdd) {
								return filter({ name: 'shan', cards: [card] }, player, event)
							} else return filter({ name: 'wuxie', cards: [card] }, player, event)
						},
						onuse(result, player) {
							player.removeMark('ba_tuolong');
							player.addTempSkill('ba_tuolong_' + result.card.name)
						},
						prerespond(result, player) {
							player.removeMark('ba_tuolong');
							player.addTempSkill('ba_tuolong_' + result.card.name)
						},
						/* onrespond(event, player){
							...
							event = event||_status.event;
							let filter=event._backup.filterCard;
							if (filter({name:'shan'},player,event)) name='shan';
							else name='wuxie';
							player.addTempSkill('ba_tuolong_'+name)
						}, */
						hiddenCard: (player, card) => {
							if (name == 'wuxie' && _status.connectMode && player.countCards('hs') > 0 && !player.hasSkill('ba_tuolong_wuxie') && player.countMark('ba_tuolong')) return true
							if (name == 'wuxie') return player.countCards('hes', card => get.number(card) % 2 == 0) && !player.hasSkill('ba_tuolong_wuxie') && player.countMark('ba_tuolong')
						},
						ai: {
							respondShan: true,
							skillTagFilter(player, tag) {
								if (player.hasSkill('ba_tuolong_shan') || player.countCards('hes', card => get.number(card) % 2 == 1) || player.countMark('ba_tuolong') == 0) return false;
								return true;
							},
							order: 2,
						},
					...
					"wuxie": {
						...
						marktext: "无懈",
						intro: {
							content: "本回合已转化过【无懈可击】"
						}
					},
				},
				mod: {
					globalFrom: (from, to, now) => {
						return now - to.countMark('ba_tuolong');
					},
				},
			},
			'ba_youwang': {
		...
		translate: {
			...
			*/
			...
			"ba_mieju_info": "结束阶段，若你回合内没有造成伤害，你可以对一名本回合内失去过牌的角色造成1点振动伤害；锁定技，当你使用牌时，你令所有与你距离为1的其他角色不能使用或打出牌响应此牌。",
			"ba_xinzhu": "心祝",
			"ba_daohui": "祷悔",
			...
			ba_mouju: "谋局",
			ba_mouju_info: "出牌阶段开始时，你可以弃置一张不为【无懈可击】的锦囊派，令一名攻击范围内的其他角色选择：弃置一张同名锦囊牌/【无懈可击】，或受到你1点伤害且直到你的下个回合开始为止，其受到的所有伤害+1，同时其受到的无属性伤害视为振动属性。",
			ba_eyan: "恶言",
			ba_eyan_info: "锁定技，当你造成伤害后，受到伤害的角色主要是所有手牌，若其中有锦囊牌，则你可以与其进行拼点，若你赢，则你获得该锦囊牌且其本回合内不得使用或打出手牌；若你没赢且此时是出牌阶段，你结束该出牌阶段。",
			ba_dingji: "定计",
			ba_dingji_info: "隐匿技，当你出场时，若当前回合角色不是你，你可以令当前回合角色选择交给你一张牌或令你从牌堆中获得一张锦囊牌。若其拒绝交牌且牌堆内没有锦囊牌，改为对其造成1点振动伤害。",
			ba_zhanhui: "展会",
			ba_zhanhui_info: "出牌阶段限一次，你可以选择至多两名其他角色，你们各亮出一张手牌，点数不为唯一最小的摸一张牌且直到其出牌阶段结束出【杀】次数+1。",
			ba_fengping: "讽评",
			ba_fengping_info: "每回合限一次，其他角色在其出牌阶段使用一张非装备牌时，你可以弃置一张牌，若其使用的牌的点数比你弃置的牌：小，则其失去1点体力；相等，则其摸两张牌且此牌不计入次数；大，则其摸一张牌，你获得此牌。",
			ba_chengpai: "成俳",
			ba_chengpai_info: "出牌阶段开始时，你可以展示一张牌堆顶的牌，然后直到你下个回合开始，当你使用或打出与该牌花色相同、或点数为5/7的倍数的牌时，你摸一张牌，然后若是在你的回合内，你的【杀】次数+1。",
			ba_qianji: "千姬",
			ba_qianji_info: "当你成为攻击范围内其他角色的牌的目标时，你可以展示任意张与之颜色相同的牌并弃置其中一张，然后令其选择：①令该牌无效且将该牌置于其武将牌上；②弃置与你展示的牌相同数量的手牌。你的回合开始时，你获得场上所有因①扣置的牌，并可对一名被获得牌的角色造成1点雷电伤害。",
			ba_poyan: "破岩",
	 ...
	 * @param  {[version1]} 版本1
	 * @param  {[version2]} 版本2
	 * @return {[number]}   若为0则相同，小于0则前者大，大于0则后者大
	 */
	get.compareVerExt = (version1, version2) => {
	...
	*/
	...
	if (get.compareVerExt(lib.version, '1.10.8') < 0) {
		lib.config.all.characters.push('ba_characters');
	}
	if (!lib.config.characters.includes('ba_characters')) lib.config.characters.push('ba_characters');
	lib.translate['ba_characters_character_config'] = '碧蓝档案';

	return ba_characters;
});